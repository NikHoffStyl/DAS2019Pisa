    ...
    for mu in muons.product() :
        ...
        if muIsTrkOrGlb :
            ...
            if mu.simFlavour() == 13 :
                ...
                histos["hSegCompPrompt"].Fill(muSegmComp)
            ...
            if mu.simFlavour() > 0 and  mu.simFlavour() < 4 :
                ...
                histos["hSegCompLightF"].Fill(muSegmComp)
            ...
            if mu.simFlavour() > 3 and  mu.simFlavour() < 6 :
                ...
                histos["hSegCompHeavyF"].Fill(muSegmComp)

        # efficiency analysis, I want to start from LOOSE (PF) muons
        if muIsLoose :

            if mu.simFlavour() == 13 :
                effs["ePtPromptMedium"].Fill(muIsMedium, muPt)
                effs["ePtPromptTight"].Fill(muIsTight, muPt)

                effs["eEtaPromptMedium"].Fill(muIsMedium, muEta)
                effs["eEtaPromptTight"].Fill(muIsTight, muEta)
            

            if mu.simFlavour() > 0 and  mu.simFlavour() < 4 :
                effs["ePtLightFMedium"].Fill(muIsMedium, muPt)
                effs["ePtLightFTight"].Fill(muIsTight, muPt)

                effs["eEtaLightFMedium"].Fill(muIsMedium, muEta)
                effs["eEtaLightFTight"].Fill(muIsTight, muEta)


            if mu.simFlavour() > 3 and  mu.simFlavour() < 6 :
                effs["ePtHeavyFMedium"].Fill(muIsMedium, muPt)
                effs["ePtHeavyFTight"].Fill(muIsTight, muPt)
